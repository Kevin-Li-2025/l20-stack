...
def should_dispatch_l20_shared_prefix_paged_decode(
    query: torch.Tensor,
    key_cache: torch.Tensor,
    block_tables: torch.Tensor,
    prefix_lengths: torch.Tensor | Sequence[int],
    *,
    ...
    if key_cache.dtype != query.dtype:
        return False
    if query.ndim != 3 or key_cache.ndim != 4:
        return False
    if block_tables.ndim != 2 or block_tables.shape[0] != query.shape[0]:
        return False
    if query.shape[0] < min_batch:
...
def maybe_l20_shared_prefix_paged_decode(
    query: torch.Tensor,
    key_cache: torch.Tensor,
    value_cache: torch.Tensor,
    block_tables: torch.Tensor,
    ...
    *,
    ...
    """Return L20 shared-prefix output for fully paged prefix and suffix."""
    if not should_dispatch_l20_shared_prefix_paged_decode(
        query,
        key_cache,
        block_tables,
        prefix_lengths,
        ...
        return None
    prefix_length = _to_int_list(prefix_lengths)[0]
    return shared_paged_prefix_paged_suffix_gqa_decode_attention(
        query,
        key_cache,
        value_cache,
        block_tables,
