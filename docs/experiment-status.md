...
| Combined sparse sampling + fused top-logprobs | Positive A100 serving A/B | `benchmarks/results/a100-vllm-combined-sampling-logprobs/` | The richer top-k/top-p + penalties + generated-token logprobs workload now wins in real vLLM HTTP serving. Against the FlashInfer sampler baseline, the no-clone raw-logits borrow candidate improves median ITL 4.388 ms -> 4.227 ms (-3.65%) and total request time 214.5 ms -> 207.6 ms (-3.21%). Against the native PyTorch sampler baseline, median ITL improves 4.549 ms -> 4.308 ms (-5.28%). Trace runs prove top-logprobs 64/64 with `borrowed` raw logits and sparse sampler 64/66 path coverage; latency uses separate no-trace runs. |
