...
| Sparse top-k/top-p + penalties | Positive A100 serving A/B | `benchmarks/results/a100-sparse-topk-topp-penalty/`, `benchmarks/results/a100-vllm-sparse-penalty-sampling/` | Sparse history keeps 1.27x-1.31x micro wins, and the opt-in vLLM path reduces A100 median ITL 9.544 ms -> 4.093 ms versus the native PyTorch sampler path. Repeat on L20, larger models, and FlashInfer-prewarmed baselines before broad production claims. |
...
sparse token-history prototype -> sparse A100 serving A/B -> L20/multi-model
serving matrix
```
