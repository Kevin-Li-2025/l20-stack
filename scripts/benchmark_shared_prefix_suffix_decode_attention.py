#!/usr/bin/env python3
"""Benchmark L20 shared-prefix plus per-request suffix decode attention."""
...
def parse_args():
    ...
    parser.add_argument("--prefix-lengths", default="1024,4096,8192")
    parser.add_argument("--suffix-lengths", default="64,256,512")
    parser.add_argument("--q-heads", type=int, default=16)
    parser.add_argument("--kv-heads", type=int, default=8)
    parser.add_argument("--head-dim", type=int, default=128)
    parser.add_argument("--baseline-split-size", type=int, default=512)
    parser.add_argument("--baseline-block-t", type=int, default=128)
    parser.add_argument("--prefix-block-ts", default="64,128")
    parser.add_argument("--prefix-block-ms", default="4,8")
...
def main() -> int:
    ...
    for batch in parse_ints(args.batches):
        for prefix_length in parse_ints(args.prefix_lengths):
            for suffix_length in parse_ints(args.suffix_lengths):
                ...
                reports.append(
                    {
                        "path": "per_request_full_split_kv",
                        "batch": batch,
                ...
                for prefix_block_t in parse_ints(args.prefix_block_ts):
                    for prefix_block_m in parse_ints(args.prefix_block_ms):
                        actual = shared_prefix_suffix_gqa_decode_attention(
                            query,
                            prefix_key,
                            prefix_value,
                            suffix_key,
                        ...
                        samples = latency_ms(
                            lambda prefix_block_t=prefix_block_t, prefix_block_m=prefix_block_m: (
                                shared_prefix_suffix_gqa_decode_attention(
                                    query,
                                    prefix_key,
                                    prefix_value,
                        ...
                        reports.append(
                            {
                                "path": "shared_prefix_suffix_merge",
                                "batch": batch,
                                "prefix_length": prefix_length,
                                "suffix_length": suffix_length,
                                "prefix_block_t": prefix_block_t,
        ...
        for prefix_length in parse_ints(args.prefix_lengths):
            for suffix_length in parse_ints(args.suffix_lengths):
                shape_reports = [
                    ...
                    and item["suffix_length"] == suffix_length
                    and item["correct"]
                ]
                baseline = next(
                    item
                ...
                candidates = [
                    item
                    for item in shape_reports
                    if item["path"] == "shared_prefix_suffix_merge"
                ]
                ...
                comparisons.append(
                    {
                        ...
                        "best_candidate_median_ms": best["median_ms"],
                        "best_prefix_block_t": best["prefix_block_t"],
                        "best_prefix_block_m": best["prefix_block_m"],
    ...
    result = {
        ...
        "shape": {
            ...
            "dtype": "bfloat16",
            "shared_prefix_plus_suffix": True,
        },
        "reports": reports,
