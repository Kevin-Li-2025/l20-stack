...
if [[ $# -ne 4 ]]; then
  cat >&2 <<'EOF'
usage: scripts/run_vllm_l20_logits_boundary_trace_campaign.sh \
  MODEL SERVED_NAME OUTPUT_DIR VLLM_SOURCE_DIR

Runs a vLLM serving campaign with the trace-only L20 logits-boundary hook.
...
"$python_bin" "$repo_root/integrations/vllm/install_l20_logits_boundary_trace.py" \
  --vllm-source "$vllm_source_dir" >/dev/null

server_log="$output_dir/server.log"
trace_jsonl="$output_dir/logits-boundary-trace.jsonl"
rm -f "$trace_jsonl"
...
setsid env \
  ...
  VLLM_L20_LOGITS_BOUNDARY_TRACE="$trace_jsonl" \
...
"$python_bin" "$repo_root/scripts/summarize_l20_logits_boundary_campaign.py" \
  "$output_dir" \
  --output-json "$output_dir/campaign-summary.json" \
