...
def test_flashsampling_shadow_plan_accepts_vllm_full_vocab_top_k(monkeypatch):
    monkeypatch.setenv("VLLM_L20_LOGITS_BOUNDARY_ALLOW_NON_L20", "1")
    module = load_module(
        "integrations/vllm/l20_flashsampling_epilogue.py",
        "l20_flashsampling_epilogue_full_vocab_topk",
    )
    batch = V2InputBatch()
    batch.top_k_cpu = np.array([151936, 151936, 999], dtype=np.int32)
    batch.top_p_cpu = np.array([1.0, 1.0, 999.0], dtype=np.float32)
    ...
    assert plan["would_use_epilogue"] is True
    assert plan["fallback_reasons"] == []
    assert plan["flashsampling_request"]["top_k"] == 151936
